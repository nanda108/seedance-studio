...
## Откуда брать промпты

Приоритет 1 — промпты уже в контексте разговора (сессия не прервалась).
Используй их напрямую — не читай файлы.

Приоритет 2 — новая сессия или контекст потерян.
Read("{project_path}/prompts/shot-1.md"), Read("{project_path}/prompts/shot-2.md") и т.д.
Если project_path неизвестен — спроси: "Укажи путь к папке проекта."
## Структура JSON
